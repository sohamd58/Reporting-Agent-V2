# importers/clevertap_channels/clevertap_push.py
import pandas as pd
import numpy as np
import re
...
def clean_clevertap_push(df: pd.DataFrame) -> pd.DataFrame:
    """
    Cleans a CleverTap Push Notification campaign export CSV.
    ...
    Total Sent, Impression, Impression %, Clicked, Clicked%, CTR, Click Through Conversion,
    Click Through Conversion %, Click through Revenue, Influenced Conversion,
    Influenced Conversion %, Influenced Revenue, Headline, Body.
    """
    df = df.copy()
    ...
    numeric_metrics = [
        ...
        "Total Clicked(users)", "Errors", "Total Unsubscribes",
        "Click through conversions", "Click through conversion revenue",
        "Unique Sent (users)", "Revenue Within Conversion Time", "Influenced Revenue",
        "Influenced Conversions"
    ]
    # Add any column that looks like a metric
    for col in df.columns:
        if any(key in col for key in ["Sent", "Delivered", "Viewed", "Clicked", "Errors", "Unsubscribes", "conversions", "revenue", "Influenced", "Impression"]):
    ...
    if "Total Sent(users)" in df.columns and "Total Viewed(users)" in df.columns:
        df["Impression %"] = (df["Total Viewed(users)"].astype(float) / df["Total Sent(users)"].astype(float) * 100).round(2)
