# importers/clevertap_channels/clevertap_whatsapp.py
import pandas as pd
import numpy as np
import re
...
def clean_clevertap_whatsapp(df: pd.DataFrame) -> pd.DataFrame:
    """
    Cleans a CleverTap WhatsApp campaign export CSV.
    ...
    Total Sent, Total Delivered, Delivery %, Total Viewed, Viewed %, Clicked, Clicked%,
    CTR, Click Through Conversion, Click Through Conversion %, Click through Revenue,
    Influenced Conversion, Influenced Conversion %, Influenced Revenue, Message.
    """
    df = df.copy()
    ...
    numeric_metrics = [
        "Total Sent(users)", "Total Delivered(users)", "Total Viewed(users)",
        "Total Clicked(users)", "Errors", "Click through conversions",
        "Click through conversion revenue", "Influenced Conversions",
        "Influenced Revenue", "estimated reach"
    ]
    # Add any column that looks like a metric
    for col in df.columns:
        if any(key in col for key in ["Sent", "Delivered", "Viewed", "Clicked", "Errors", "conversions", "revenue", "Influenced", "reach"]):
    ...
    if "Total Delivered(users)" in df.columns and "Total Viewed(users)" in df.columns:
        df["Viewed %"] = (df["Total Viewed(users)"].astype(float) / df["Total Delivered(users)"].astype(float) * 100).round(2)
    ...
    if "Total Viewed(users)" in df.columns and "Total Clicked(users)" in df.columns:
    ...
    rename_map = {
        ...
        "Total Viewed(users)": "Total Viewed",
